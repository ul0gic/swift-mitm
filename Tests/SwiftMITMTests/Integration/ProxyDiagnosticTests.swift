...
final class ProxyDiagnosticTests: XCTestCase {
    private func runTransfer(bodySize: Int) throws -> Int {
        ...
        client.armWatchdog(seconds: 120)
    ...
    func testTiny1KB() throws {
        XCTAssertEqual(try runTransfer(bodySize: 1024), 1024)
    }

    func testTwoWindows128KB() throws {
        XCTAssertEqual(try runTransfer(bodySize: 128 * 1024), 128 * 1024)
    }

    func testOneMB() throws {
        XCTAssertEqual(try runTransfer(bodySize: 1024 * 1024), 1024 * 1024)
    }

    func testSixteenMB() throws {
        XCTAssertEqual(try runTransfer(bodySize: 16 * 1024 * 1024), 16 * 1024 * 1024)
    }
}
